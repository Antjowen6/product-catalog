...
def create_or_update_product(product: ProductCreate, x_api_key: str = Header(default=None)):
    verify_api_key(x_api_key)
    conn = get_db()
...
def get_product(sku: str, x_api_key: str = Header(default=None)):
    verify_api_key(x_api_key)
    conn = get_db()
...
def update_product(sku: str, updates: ProductUpdate, x_api_key: str = Header(default=None)):
    verify_api_key(x_api_key)
    conn = get_db()
    row = conn.execute("SELECT * FROM products WHERE sku = ?", (sku,)).fetchone()
    if not row:
...
def delete_product(sku: str, x_api_key: str = Header(default=None)):
    verify_api_key(x_api_key)
    conn = get_db()
    row = conn.execute("SELECT * FROM products WHERE sku = ?", (sku,)).fetchone()
    if not row:
...
async def push_to_shopify(sku: str, x_api_key: str = Header(default=None)):
    verify_api_key(x_api_key)
    ...
    shopify_data = {
        "product": {
            ...
            "variants": [{"sku": product["sku"], "price": str(product["price"]), "inventory_management": "shopify"}],
        }
    }
    ...
    async with httpx.AsyncClient() as client:
        if shopify_product_id:
            resp = await client.put(f"{base_url}/products/{shopify_product_id}.json", json=shopify_data, headers=headers)
        else:
            resp = await client.post(f"{base_url}/products.json", json=shopify_data, headers=headers)
        if resp.status_code not in (200, 201):
            raise HTTPException(status_code=502, detail=f"Shopify API error ({resp.status_code}): {resp.text[:300]}")
        result = resp.json()
        new_shopify_id = str(result["product"]["id"])
        now = datetime.utcnow().isoformat()
        conn.execute("UPDATE products SET in_shopify = 1, shopify_product_id = ?, updated_at = ? WHERE sku = ?", (new_shopify_id, now, sku))
        conn.commit()
        conn.close()
        return {"status": "pushed", "sku": sku, "shopify_product_id": new_shopify_id, "shopify_url": f"https://{SHOPIFY_STORE}/admin/products/{new_shopify_id}"}

@app.get("/shopify/status")
def shopify_status(x_api_key: str = Header(default=None)):
    verify_api_key(x_api_key)
    return {"configured": bool(SHOPIFY_STORE and SHOPIFY_TOKEN), "store": SHOPIFY_STORE if SHOPIFY_STORE else None}
